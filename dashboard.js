document.addEventListener('DOMContentLoaded', () => {
  ...
  const captureEngine = new window.ScreenCaptureEngine();
  const changeDetector = new window.ChangeDetector();
  const ocrEngine = new window.OCREngine();
  const logger = new window.MicrosecondLogger();
  const testSuite = new window.AcceptanceTestSuite(captureEngine, changeDetector, ocrEngine, logger);
